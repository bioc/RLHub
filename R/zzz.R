.onLoad <- function(libname, pkgname) {
    ...
    createHubAccessors2 <- function(pkgname, titles, tags) {
        ...
        haf <- utils::getFromNamespace(".hubAccessorFactory", "ExperimentHub")
        eh <- AnnotationHub::query(geh(), pkgname)
    ...
    tags <- utils::read.csv(fl, stringsAsFactors = FALSE)$Tag
